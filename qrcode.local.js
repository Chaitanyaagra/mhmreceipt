...
/* ----------------------------------------------------------------------------
   Adapter: expose the same window.QRCode.toDataURL(text, opts, cb) API that the
   app used to load from the CDN, so nothing else in the app has to change.
   Renders the QR to a <canvas> with the requested size and colours and returns
   a PNG data-URL. Self-contained — no network needed.
   ---------------------------------------------------------------------------- */
(function () {
  function toDataURL(text, opts, cb) {
    ...
    try {
      ...
      // Build the QR (error-correction level M, auto type number).
      var qr = qrcode(0, 'M');
